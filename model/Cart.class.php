<?php
  ...
  class Cart{
    ...
    public function __construct(){
      // get the shopping cart array from the session_start
      $this->cart_contents=!empty($_SESSION['cart_contents'])?$_SESSION['cart_contents']:NULL;
      if($this ->cart_contents ===NULL){
        // set some base values
        $this ->cart_contents = array('cart_contents'=>0, 'total_items'=>0);
      }
    }
    /**
    ...
    public function insert($item = array()){
        if(!is_array($item) OR count($item) === 0){
            ...
            if(!isset($item['id'], $item['title'], $item['price'], $item['quantity'])){
                ...
                $old_qty = isset($this->cart_contents[$rowid]['quantity']) ? (int) $this->cart_contents[$rowid]['quantity'] : 0;
